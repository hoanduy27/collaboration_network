...
def __modularity(status, resolution):
    ...
    for community in set(status.node2com.values()):
        ...
        com_size = len([node for node in status.node2com if status.node2com[node] == community])
        my_resolution = in_degree / ( resolution * com_size * (com_size - 1)) if com_size > 1 else 0
        if links > 0:
            result += in_degree * my_resolution / links -  ((degree * my_resolution/ (2. * links)) ** 2)
    return result
